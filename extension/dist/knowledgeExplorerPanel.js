...
exports.KnowledgeExplorerPanel = void 0;
const vscode = __importStar(require("vscode"));
const STATE_KEY = 'pdfKnowledge.knowledgeExplorer.state';
const DEFAULT_TOPIC = 'Show me the overall knowledge map and the major concepts I should learn first.';
class KnowledgeExplorerPanel {
    ...
    seedTopic;
    static currentPanel;
    static createOrShow(context, service, seedTopic = '') {
        ...
        const panel = vscode.window.createWebviewPanel('pdfKnowledge.knowledgeExplorer', 'PDF Knowledge Explorer', column, {
            enableScripts: true,
        ...
        KnowledgeExplorerPanel.currentPanel = new KnowledgeExplorerPanel(panel, context, service, seedTopic);
    }
    state;
    disposed = false;
    autoMapStarted = false;
    constructor(panel, context, service, seedTopic) {
        this.panel = panel;
        this.context = context;
        this.service = service;
        this.seedTopic = seedTopic;
        this.state =
    ...
    async generateMap(topic, level, mode) {
        this.post({
            type: 'busy',
            busy: true,
            status: 'Starting Knowledge Explorer...'
        });
        const map = await this.service.createMap(topic, level, mode, status => this.post({ type: 'progress', status }));
        this.state = {
            map,
    ...
    async exploreNode(nodeId) {
        ...
        const detail = await this.service.exploreNode(this.state.map, nodeId, this.state.learningLevel, this.state.retrievalMode, status => this.post({ type: 'progress', status }));
        this.state = {
            ...this.state,
    ...
    async exploreQuestion(question) {
        if (!this.state.map) {
        ...
        const detail = await this.service.exploreQuestion(this.state.map, question.trim(), this.state.learningLevel, this.state.retrievalMode, status => this.post({ type: 'progress', status }));
        this.state = {
            ...this.state,
            ...
            switch (message.type) {
                case 'ready': {
                    ...
                    if (!this.state.map && !this.autoMapStarted) {
                        ...
                        await this.generateMap(this.seedTopic.trim() || DEFAULT_TOPIC, this.state.learningLevel, this.state.retrievalMode);
                    }
                    break;
                }
                case 'generateMap':
                    await this.generateMap(String(message.topic ?? DEFAULT_TOPIC), String(message.learningLevel ?? 'New to the topic'), this.mode(message.mode));
                    break;
                case 'exploreNode':
                ...
                case 'askChat': {
                    ...
                    await vscode.commands.executeCommand('workbench.action.chat.open', { query: `@pdf-knowledge ${query}` });
                    break;
                }
                case 'clear':
...
exports.KnowledgeExplorerPanel = KnowledgeExplorerPanel;
//# sourceMappingURL=knowledgeExplorerPanel.js.map