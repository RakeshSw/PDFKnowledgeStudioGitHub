...
function coverageScore(value) {
    return { high: 1, medium: 0.65, low: 0.25 }[value.toLowerCase()] ?? 0.4;
}
...
function subjectIntentScore(record, kind, subjects) {
    ...
    if (subjects.has('functions')) {
        ...
        if (present === 6)
            intent += 2.2;
        if (key.includes('resource_and_overview'))
            intent += 2;
        if (key.includes('core_framework'))
            intent += 1.8;
        if (titleTopic.includes('functions'))
            intent += 1.2;
        if (titleTopic.includes('glossary'))
            intent += 0.6;
    }
    if (subjects.has('organizational_profile')) {
        if (key.includes('creating_and_using_organizational_profiles'))
            intent += 5;
        if (titleTopic.includes('organizational profile'))
            intent += 2.2;
        if (key.includes('core_framework'))
            intent += 1.4;
        if (key.includes('cybersecurity_erm_and_workforce'))
            intent += 0.8;
        if (key.includes('informative_references'))
            intent += 0.6;
        if (titleTopic.includes('community profile'))
            mismatch += 2.5;
        if (key.includes('incident_response'))
            mismatch += 1.8;
        if (key.includes('ransomware'))
            mismatch += 2;
        if (key.includes('supply_chain'))
            mismatch += 1.5;
    }
    if (subjects.has('community_profile')) {
        if (key.includes('creating_community_profiles'))
            intent += 5;
        if (titleTopic.includes('community profile'))
            intent += 2.2;
        if (titleTopic.includes('organizational profile'))
    ...
    if (subjects.has('tiers')) {
        if (key.includes('using_the_csf_tiers'))
            intent += 5;
        if (titleTopic.includes('tiers'))
            intent += 2;
        if (key.includes('core_framework'))
            intent += 1.2;
    }
    if (subjects.has('informative_references')) {
        if (key.includes('informative_references'))
            intent += 5;
        if (titleTopic.includes('informative reference'))
            intent += 2;
        if (key.includes('core_framework') || key.includes('resource_and_overview'))
            intent += 1;
    }
    if (subjects.has('incident_response')) {
        if (key.includes('incident_response'))
            intent += 5;
        if (titleTopic.includes('incident response'))
            intent += 2;
        if (key.includes('core_framework'))
            intent += 0.8;
    }
    if (subjects.has('supply_chain')) {
        if (key.includes('supply_chain_risk_management'))
            intent += 5;
        if (titleTopic.includes('supply chain') || titleTopic.includes('c-scrm'))
            intent += 2;
        if (key.includes('core_framework'))
            intent += 0.8;
    }
    if (subjects.has('ransomware')) {
        if (key.includes('ransomware_risk_management'))
            intent += 5;
        if (titleTopic.includes('ransomware'))
            intent += 2;
        if (key.includes('incident_response'))
            intent += 0.8;
    }
    if (subjects.has('erm')) {
        if (key.includes('enterprise_risk_management'))
            intent += 5;
        if (titleTopic.includes('enterprise risk management'))
            intent += 2;
        if (key.includes('cybersecurity_erm_and_workforce'))
            intent += 1.5;
    }
    if (subjects.has('workforce')) {
        if (key.includes('cybersecurity_erm_and_workforce'))
            intent += 5;
        if (titleTopic.includes('workforce'))
            intent += 2;
    }
    if (subjects.has('small_business')) {
        if (key.includes('small_business'))
            intent += 5;
        if (titleTopic.includes('small business'))
            intent += 2;
    }
    ...
    if (key.includes('small_business') && !subjects.has('small_business'))
        mismatch += 1.3 * scale;
    if (key.includes('incident_response') && !subjects.has('incident_response'))
        mismatch += 1.4 * scale;
    if (key.includes('ransomware') && !subjects.has('ransomware'))
        mismatch += 1.8 * scale;
    if (key.includes('supply_chain') && !subjects.has('supply_chain'))
        mismatch += 1.1 * scale;
    if (key.includes('creating_community_profiles') && !subjects.has('community_profile'))
        mismatch += 1.2 * scale;
...
function relevancePolicy(kind) {
    if (kind === 'direct')
        return { floor: 0.5, minimumKeep: 4, perDocumentLimit: 3 };
    if (kind === 'process')
        return { floor: 0.38, minimumKeep: 6, perDocumentLimit: 4 };
    if (kind === 'document')
        return { floor: 0.2, minimumKeep: 12, perDocumentLimit: 5 };
    return { floor: 0.32, minimumKeep: 6, perDocumentLimit: 4 };
}
function adaptiveFinalK(mode, kind) {
    if (mode === 'fast') {
        return { direct: 6, process: 10, general: 8, document: 14 }[kind];
    }
    return { direct: 8, process: 12, general: 10, document: 22 }[kind];
}
function modeParameters(mode, kind, overrideFinalK) {
...
function buildContextPack(query, selected, maxChars) {
    const header = [
...
class RetrievalClient {
    ...
    async ensureLoaded() {
        ...
        this.loading = (async () => {
            ...
            const loaded = lines.map((line, index) => {
                try {
                    return indexRecord(JSON.parse(line));
                }
                catch (error) {
    ...
    fieldScore(record, querySet) {
        if (!querySet.size)
            return 0;
        const overlap = (tokens) => intersectionSize(querySet, tokens) / querySet.size;
    ...
    phraseScore(record, query, queryTokens) {
        const text = record.allText.toLowerCase();
        const title = record.title.toLowerCase();
        const topic = record.topic.toLowerCase();
        const folded = query.toLowerCase().trim();
        let score = 0;
        if (folded.length >= 5 && text.includes(folded))
            score += 1.4;
        if (folded.length >= 5 && title.includes(folded))
            score += 2.2;
        if (folded.length >= 5 && topic.includes(folded))
            score += 1.6;
        const meaningful = queryTokens.filter(token => token.length >= 3);
        for (const [size, value] of [[4, 0.8], [3, 0.55], [2, 0.3]]) {
            if (meaningful.length < size)
                continue;
            let hits = 0;
            for (let i = 0; i <= meaningful.length - size; i += 1) {
                if (text.includes(meaningful.slice(i, i + size).join(' ')))
                    hits += 1;
            }
    ...
    search(query, mode, overrideFinalK) {
        ...
        for (const record of this.records ?? []) {
            ...
            if (!overlapCount && intent <= 0)
                continue;
            if (coverageRatio < 0.1 && intent < 1 && bm25 < 0.35)
                continue;
        ...
        while (remaining.length && selected.length < parameters.final_k) {
            ...
            for (let i = 0; i < remaining.length; i += 1) {
                ...
                if ((documentCounts.get(documentId) ?? 0) >= policy.perDocumentLimit)
                    continue;
    ...
    async retrieve(query, mode, cancellationToken, finalK) {
        ...
        const records = selected.map((item, index) => ({
            ...
            score_breakdown: Object.fromEntries(Object.entries(item.breakdown).map(([key, value]) => [key, Number(value.toFixed(6))]))
        }));
        return {
...
//# sourceMappingURL=retrievalClient.js.map