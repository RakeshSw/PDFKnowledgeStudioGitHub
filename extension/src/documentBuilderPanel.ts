...
import { DocumentState, RetrievalMode } from './types';

const STATE_KEY = 'pdfKnowledge.documentBuilder.state';

export class DocumentBuilderPanel {
    private static currentPanel: DocumentBuilderPanel | undefined;
    ...
    public static createOrShow(
        ...
        DocumentBuilderPanel.currentPanel = new DocumentBuilderPanel(
            panel,
            context,
            documentService,
            seedPrompt
    ...
    private constructor(
        ...
        this.state = context.workspaceState.get<DocumentState>(STATE_KEY);
        ...
        try {
            switch (message.type) {
                ...
                case 'generate': {
                    ...
                    const mode = (message.mode === 'fast' ? 'fast' : 'deep') as RetrievalMode;
                    this.post({ type: 'busy', busy: true, status: 'Starting document generation...' });
                    this.state = await this.documentService.generate(
                        prompt,
                        String(message.audience ?? ''),
                        String(message.documentType ?? ''),
                        mode,
                ...
                case 'addSection': {
                    ...
                    this.state = await this.documentService.addSection(
                        this.state,
                        request,
                        mode,
                        status => this.post({ type: 'progress', status })
                ...
                case 'generateDiagram': {
                    ...
                    this.state = await this.documentService.addDiagram(
                        this.state,
    ...
    private async saveMarkdown(state: DocumentState): Promise<void> {
    ...
    private html(webview: vscode.Webview, seedPrompt: string): string {
