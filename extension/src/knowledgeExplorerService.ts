...
import {
    ...
    KnowledgeMapNode,
    KnowledgeNodeDetail,
    KnowledgeRelatedConcept,
    QueryPlan,
    RetrievalMode,
    RetrievalSource
} from './types';

interface RawKnowledgeMapNode {
    id?: unknown;
    label?: unknown;
    category?: unknown;
    summary?: unknown;
    explorationQuery?: unknown;
    evidenceNumbers?: unknown;
}

interface RawKnowledgeMapEdge {
    from?: unknown;
    to?: unknown;
    label?: unknown;
}
...
interface RawKnowledgeNodeDetail {
    title?: unknown;
    category?: unknown;
    overview?: unknown;
    ...
    keyIdeas?: unknown;
    steps?: unknown;
    commonMisunderstandings?: unknown;
    relatedConcepts?: unknown;
    suggestedQuestions?: unknown;
    visualTitle?: unknown;
    visualMermaid?: unknown;
    knowledgeCheck?: unknown;
}

const DEFAULT_EXPLORER_TOPIC =
    'Create an overall map of the major concepts, implementation guidance, profiles, risks, and supporting practices represented in the knowledge base.';
...
function evidenceSources(
    ...
    const selected = evidence.records.filter(record =>
        evidenceNumbers.includes(record.evidenceNumber)
    );
    const fallback = evidence.records.slice(0, 2);
    return uniqueSources(
        (selected.length ? selected : fallback).map(record => record.source)
    );
}

function normalizeEvidenceNumbers(value: unknown, evidence: CombinedEvidence): number[] {
...
function buildMapMermaid(nodes: KnowledgeMapNode[], edges: KnowledgeMapEdge[]): string {
...
function buildOverallPlan(topic: string): QueryPlan {
...
function normalizeMap(
    ...
    const rawNodes = Array.isArray(raw.nodes)
        ? raw.nodes as RawKnowledgeMapNode[]
        : [];
    ...
    for (const item of rawNodes.slice(0, 12)) {
        ...
        nodes.push({
            ...
            explorationQuery: text(
                item.explorationQuery,
                `Explain ${label}, why it matters, how it works, and how it relates to the wider knowledge domain.`
            ).slice(0, 500),
            evidenceNumbers,
    ...
    const rawEdges = Array.isArray(raw.edges)
        ? raw.edges as RawKnowledgeMapEdge[]
        : [];
    ...
    return {
        ...
        overview: text(
            raw.overview,
            'Explore the major evidence-grounded concepts and follow their relationships.'
        ),
...
function normalizeRelatedConcepts(value: unknown): KnowledgeRelatedConcept[] {
    if (!Array.isArray(value)) {
        return [];
    }
    const result: KnowledgeRelatedConcept[] = [];
...
function normalizeKnowledgeCheck(value: unknown): KnowledgeCheck | undefined {
...
function normalizeNodeDetail(
    raw: RawKnowledgeNodeDetail,
...
export class KnowledgeExplorerService {
    public constructor(
    ...
    public async createMap(
        ...
        try {
            const languagePlan = await this.orchestrator.plan(
                mapTopic,
                [],
                cancellationToken
            );
        ...
        const prompt = await loadPrompt(this.extensionUri, 'knowledge-map.md');
        const raw = await this.azure.completeJson<RawKnowledgeMap>(
            [
    ...
    public async exploreNode(
        ...
        const plan = await this.orchestrator.plan(
            node.explorationQuery,
            [map.topic, map.overview],
            cancellationToken
        );
        ...
        const prompt = await loadPrompt(this.extensionUri, 'knowledge-node.md');
        const raw = await this.azure.completeJson<RawKnowledgeNodeDetail>(
            [
                { role: 'system', content: prompt },
                {
                    role: 'user',
    ...
    public async exploreQuestion(
        ...
        const syntheticNode: KnowledgeMapNode = {
            id: syntheticId,
        ...
        const extendedMap: KnowledgeMap = {
            ...map,
    ...
    public initialState(): KnowledgeExplorerState {
        return {
